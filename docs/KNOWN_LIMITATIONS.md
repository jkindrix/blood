...
### GAP-2: `Frozen<T>` deep traversal — partially resolved (A1 Phase 1+2+3a+3b in place; Phase 3c/3d pending)
...
**A1 Phase 3 remaining (c/d):** (c) Full stdlib Frozen wire-up once the `use std.core.frozen` import path exists (removes the "every Frozen golden declares its own struct" friction). (d) `Freeze` trait bound integration — emit layout descriptors only for `T: Freeze`, catch misuse at compile time. (b-followup) User-defined enum variants with generic payloads — requires threading `TypeArgs` through variant field resolution.
