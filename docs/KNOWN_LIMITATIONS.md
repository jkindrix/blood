...
**Note (previously out of scope, now closed in session 62):** ~~`string_as_bytes` codegen still uses `blood_get_generation` (lookup-only, returns 0 for unregistered) — &[u8] from `as_bytes()` remains untracked.~~ `emit_string_as_bytes_call` now calls `@blood_lazy_register_gen` mirroring the S51 treatment in `emit_string_as_str_call`; &[u8] from `as_bytes()` tracks its data buffer's generation through `blood_free_simple`. Stale-ref detection via deref is now symmetric for both &str and &[u8]. `blood_realloc`'s registration asymmetry (new buffer via sys_calloc bypasses registration) is unchanged; rt_string/rt_vec growth uses `alloc_simple + free_simple` directly, not `blood_realloc`, so the asymmetry doesn't affect String/Vec stale detection.

### GAP-2: `Frozen<T>` deep traversal — partially resolved (A1 Phase 1+2+3a+3b+3d in place; Phase 3c / 3b-followup pending)

`blood_freeze()` marks only the root allocation as frozen (gen set to `0x7FFFFFFE`). The A1 arc wires deep-freeze traversal so inner heap pointers (String data, Vec data, Box pointees, nested-struct subfields) also get frozen.

**Cycle guard in place:** `blood_freeze` checks whether an allocation is already frozen before re-marking it; `blood_freeze_fields` mirrors the check on its kind=3 recursion path. Together these prevent infinite recursion on cyclic Box-graph heap structures.

**A1 Phase 1 (session 57):** codegen plumbs per-inner-type layout-descriptor globals (`@blood_layout.N`) and a `call i1 @blood_freeze_fields(ptr %persisted, ptr @blood_layout.N)` alongside `blood_freeze`, end-to-end through the batch + streaming + parallel-worker merge paths. Phase 1 descriptors carried `num_entries == 0` — the runtime handler validated the header and returned success; no fields were walked.

**A1 Phase 2 (session 58):** descriptors are now populated with real field entries. Entry layout: `{ i32 offset, i32 kind, ptr inner_layout, i64 element_size }` (24 bytes). Kinds: 1 = String field (follows `data` ptr, lazy-registers the buffer, calls `blood_freeze`); 2 = Vec<T> field (freezes the element buffer, then walks elements at `element_size` stride via recursive `blood_freeze_fields(elem_ptr, inner_layout)`); 3 = Box<T> field (dereferences, freezes pointee, recurses); 4 = inline nested struct (recurse in place). Classification walks ADT fields via `adt_registry`; inner_layout emits as `null` when the inner type has no heap-owning subparts. Inline-vs-indirect dispatch at Frozen aggregate site: if `sizeof(Frozen<T>) < sizeof(T)`, emit a "kind=3 wrapper" descriptor that dereferences the pointer; else walk T's fields at offset 0. Also: the Frozen codegen path now routes through user-defined `struct Frozen<T>` (previously an orphan builtin DefId) via `register_or_use_existing_builtin` in hir_lower_builtin.blood. Runtime walker lazy-registers String/Vec data buffers before freezing (they come from `blood_alloc_simple` which doesn't gen-track by default). Observable golden: `t05_frozen_deep_string.blood` — wraps a `struct Wrapper { s: String }` in `Frozen<T>`, reads the inner String's data-buffer pointer via `@unsafe`, calls `blood_is_frozen` via FFI bridge, and asserts 1.

**A1 Phase 3a (session 60, 2026-04-23):** SHIPPED via MIR intrinsic. HIR lowering's `lower_assoc_fn` captures the DefId of every `Frozen.new` impl method (matched by `impl_type_def_id == ctx.builtin_frozen_def && fn_name_sym == intern("new")`) into `ctx.frozen_new_def_ids: Vec<u32>`. The Vec is propagated to `LowerResult` and threaded into `MirLowerCtx` at every top-level lowering site (driver Phase 4b/4c, main.blood codegen workers + sequential pass, mono-request specialization paths). At `mir_lower_expr.lower_call_expr`, before the regular `terminate_call`, the new `if ctx.is_frozen_new(path.def_id.index) && args.len() == 1` branch emits `Rvalue.Aggregate(AggregateKind.Adt { def_id: extract_adt_def_id(ty), variant_idx: 0, type_args: extract_type_args(resolved_ty) }, [arg_operand])` inline — using the call's return type's `type_args[0]` as the concrete T — and returns early. Codegen's existing ADT aggregate path then handles persist + freeze + freeze_fields with the concrete T from the call site (the same Phase 2 path used by inline `Frozen { value: w }`). The closure-context propagation copies the table from parent ctx so nested closures intercept correctly too. Scope: 7 source files, ~70 added lines. Bounded to Frozen — other `impl<T>` methods retain their current behavior (including the latent multi-caller bug described next). Three-gen byte-identical bootstrap at seed `4ea16b34` (14,370,376 bytes, +20,528 vs. S59 `5eb60ecd`). Runtime archive unchanged at `4d9bb4be`. 582/582 goldens under first_gen and second_gen — t05_frozen_new_heap_shallow.blood now outputs `1` (was the regression guard `EXPECT: 0`; rewritten to assert the fix).

**A1 Phase 3b (session 61, 2026-04-23):** SHIPPED. Kind=5 enum-dispatch entries are now emitted for `Frozen<Option<T>>` and `Frozen<Result<T, E>>` when any variant carries a heap-owning payload. The walker (`rt_blood_freeze_fields` in `runtime/blood-runtime/alloc.blood`) reads the i32 discriminant at `enum_addr+0`, looks up the matching variant in a 24-byte-stride variant table (reuses the standard `{ i32, i32, ptr, i64 }` entry shape with `offset` repurposed as the variant tag and `inner_label` as a sublayout pointer), and recurses with the variant's sublayout rooted at the enum's address — sublayout entries carry payload-relative offsets (payload starts at byte 8: i32 disc + 4 bytes pad for the `[N x i64]` payload slot's 8-byte alignment). Variant table entries for unit variants (`Option.None`, any unit variant) or non-heap-payload variants (`Result<String, i32>`'s Err path) emit a NULL sublayout pointer and the walker no-ops on that discriminant value. Codegen: `src/selfhost/codegen_ctx.blood` adds `option_def_id` / `result_def_id` fields (wired in `main.blood` from `LowerResult.builtin_{option,result}_def`) plus two methods — `add_variant_table(enum_ty_id_idx, entries)` (dedup key `ty_id_idx | 0x4000000000000000` to avoid colliding with regular descriptors or indirect wrappers) and `add_anonymous_sublayout(entries)` for per-variant sublayouts; `src/selfhost/codegen.blood`'s `compute_layout_entries`, `classify_field_to_entry`, and `ty_has_heap_owning_parts_depth` each gain an Option/Result branch plus a shared `build_builtin_enum_variant_table` / `build_variant_payload_sublayout` helper pair that delegates back to `classify_field_to_entry` for the payload type (handles String / Vec / Box / nested inline struct / nested Option / nested Result uniformly). Scope: 8 source files + 2 goldens, ~190 lines. User-defined enum variants remain deferred to a future Phase 3b follow-up — those require type-arg substitution through `VariantLayout.fields.hir_type` which is not threaded through the current codegen path. Observable goldens: `t05_frozen_option_string.blood` (Frozen<Option<String>> — Some's inner String is frozen via kind=5 → variant sublayout kind=1 at offset 8) and `t05_frozen_result_ok_string.blood` (Frozen<Result<String, i32>> — Ok's String is frozen; Err would no-op via NULL sublayout).

**A1 Phase 3d minimum (session 62, 2026-04-23):** SHIPPED. Impl-level trait bounds are now enforced at path-call sites — `register_fn_sig_with_impl_generics` in `src/selfhost/typeck.blood` merges `impl<T: Freeze> Frozen<T>`'s `T: Freeze` predicate into the method's `FnSigInfo.where_predicates` so that `infer_call → check_where_clause_bounds` sees and enforces it against the resolved arg type. Before the fix, the impl-level bound was silently dropped at the method registration boundary and `Frozen.new(&mut i32)` typeck'd successfully despite the marker-trait violation. `type_implements_trait` was additionally reordered to consult user-declared manual impls first (e.g., `impl Freeze for Wrapper {}`) before the structural auto-derive in `type_is_freeze` — the old order let auto-derive override intentional user impls for types whose fields include builtin ADTs (like String) whose struct info is `None`. Negative golden: `tests/golden/t06_err_frozen_new_mut_ref.blood` — `Frozen.new(&mut i32)` now rejects at typeck with E0206. 585/585 goldens pass first_gen + second_gen; gate --quick byte-identical third_gen at seed `4a3ed1dd` (14,395,184 bytes).

**A1 Phase 3d full (session 63, 2026-04-23):** SHIPPED (verification-only — no compiler change required). S62's Phase 3d minimum combined with the pre-existing structural walk in `type_is_freeze` (typeck.blood:2249-2292) already covers auto-derive for user structs with all-Freeze fields: (1) impl-level `T: Freeze` is merged into the method's `FnSigInfo.where_predicates`; (2) `check_where_clause_bounds` calls `type_implements_trait(arg_type, Freeze)`; (3) `type_implements_trait` consults manual impls first (none matches); (4) the auto-derive branch calls `type_is_freeze`, whose Adt arm walks the struct's fields recursively. For `struct Point { x: i32, y: i32 }` both fields are primitive (Freeze-by-construction), so auto-derive returns true without any `impl Freeze for Point {}` boilerplate. Positive golden: `tests/golden/t05_frozen_auto_derive_struct.blood` — `Frozen.new(Point { x: 42, y: 99 })` compiles and runs, asserting field access via `get()`. The complementary negative path is already covered by `t06_err_freeze_non_freeze_field.blood` (struct with `&mut i32` field fails via generic-fn bound) and `t06_err_frozen_new_mut_ref.blood` (bare `&mut i32` fails via impl-method bound). 586/586 goldens pass first_gen + second_gen; gate --quick byte-identical third_gen at seed `4a3ed1dd` (size unchanged — golden-only change).

**A1 Phase 3 remaining (c / b-followup):** (c) Full stdlib Frozen wire-up once the `use std.core.frozen` import path exists (removes the "every Frozen golden declares its own struct" friction; requires relocating the builtin Frozen DefId assignment from Phase 1b to after Phase 3a stdlib-module loading, plus re-running S60's `lower_assoc_fn` capture against the rebound DefId). (b-followup) User-defined enum variants with generic payloads — requires threading `TypeArgs` through `VariantLayout.fields.hir_type` so that `Frozen<MyResult<String, i32>>` is walked correctly.

**Latent related issue (NOT addressed by Phase 3a):** The MIR-intrinsic approach is targeted; it does not fix the underlying impl-level type-param monomorphization gap for general `impl<T>` methods. `Holder<i32>` and `Holder<String>` instantiated in the same scope still race on `body_tv` (the impl's TypeParam.def.var_id is shared across all callers, last-set wins) and produce a typeck error when the second caller's `unwrap()` is dispatched against the first caller's bound T. Zero existing goldens exercise this. A proper "impl-level generic monomorphization" fix would handle body-side TyVarId consistency in `compile_monomorphized_bodies` and is multi-session work — tracked under `.tmp/BUGS_OPEN.md` A1-P3a "Latent related issue".
