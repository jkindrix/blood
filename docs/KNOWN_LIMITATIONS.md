...
### GAP-2: `Frozen<T>` deep traversal — partially resolved (A1 Phase 1+2 in place; Phase 3 pending)
...
**A1 Phase 3 (future):** (a) lift the Phase 2 `sizeof(Frozen<T>) < sizeof(T)` inline-vs-indirect heuristic to a proper type-preserving generic monomorphization pipeline so that `Frozen.new(w)` (through the generic constructor method) sees `T=Wrapper` concretely rather than as the erased `i64`; Phase 2 works around this by having the new golden construct the `Frozen { value: w }` aggregate inline (where type info is preserved), while existing `Frozen.new` goldens still hit the erased path and their zero-entry descriptors remain accurate no-ops. (b) Enum variants with heap fields (kind=5 with per-variant payload layouts). (c) Full stdlib Frozen wire-up once the `use std.core.frozen` import path exists. (d) `Freeze` trait bound integration — emit layout descriptors only for `T: Freeze`, catch misuse at compile time.

**Session 59 investigation (2026-04-23):** Root cause of (a) above is localized. Blood's impl-level type params in `impl<T> Frozen<T>` aren't per-call-monomorphized: `driver.collect_type_generic_fns` only inspects method-level generics (`&assoc_fn.sig.generics`), and `typeck_expr.lower_method_call_expr`'s fresh-var loop starts at `method_start = impl_gc`. Result: `fn_sig.num_generics > impl_gc` is false for impl-only generic methods, no `TypeGenericCallInfo` recorded, no `MonoRequest` emitted, single body compiled with `T → Infer(body_tv)` defaulting to i64 at codegen. For T > 8 bytes: caller truncates to i64 at call, body persists only 8 bytes, `blood_freeze_fields` sees zero-entry descriptor from `type_args[0] = Infer(53)`. The "broad fix" (include impl-level in both the `fn_set` count and the freshening loop, method_start → 0) was attempted in S59: it correctly produced a `blood_mono$def24$T25` specialization routed by the caller, but the mono body's inner Aggregate `args=<?53>` stayed unresolved (subst binding key didn't match the body's Infer var_id), and the `_2: ?53` temp's alloca stayed as i64, causing a 24→8 byte truncation at the internal `store` site. Four other goldens (`t05_frozen_generic`, `t05_generic_impl_method_on_instance`, `t05_generic_struct_mixed_fields`, `t05_generic_struct_multi_field`) regressed because my subst binding added `body_tv → fresh_var` locally, which broke downstream `substitute_type_params_id` flow for `get()`-style methods that had been accidentally correct under the erased-i64 path. Regression guard `tests/golden/t05_frozen_new_heap_shallow.blood` codifies the current broken behavior (`EXPECT: 0`); when Phase 3 (a) lands it will output `1` and force the guard's removal.

**Recommended Phase 3 (a) approach:** MIR intrinsic — detect Frozen.new's DefId at HIR lowering (post-impl scan for `impl<T> Frozen<T> { fn new(...) }` pattern), store in `LowerResult.frozen_new_def_ids`, thread through to `MirLowerCtx`. At `mir_lower_expr.lower_call_expr`, before emitting the `Call` terminator, if `callee.def_id ∈ frozen_new_def_ids`, emit `Rvalue::Aggregate(Adt(frozen_def_id, 0, [T]), [value])` inline using the call's return type's `type_args[0]` as concrete T. Bypasses the generic body entirely. Scope: ~50 lines, bounded blast radius (Frozen-specific). Does not fix the underlying "multi-caller impl<T> races on body_tv" soundness issue for other impl<T> methods — that stays as a latent follow-up tracked under a future GAP ("impl-level generic monomorphization").
