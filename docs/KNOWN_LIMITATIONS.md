...
### GAP-2: `Frozen<T>` deep traversal — partially resolved (A1 Phase 1+2+3a in place; Phase 3b–d pending)
...
**A1 Phase 3 remaining (b/c/d):** (b) Enum variants with heap fields (kind=5 with per-variant payload layouts). (c) Full stdlib Frozen wire-up once the `use std.core.frozen` import path exists. (d) `Freeze` trait bound integration — emit layout descriptors only for `T: Freeze`, catch misuse at compile time.
