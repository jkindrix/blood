...
### GAP-2: `Frozen<T>` deep traversal — partially resolved (A1 Phase 1+2+3a+3b+3d-min in place; Phase 3c / 3d-full pending)
...
**A1 Phase 3 remaining (c / d-full / b-followup):** (c) Full stdlib Frozen wire-up once the `use std.core.frozen` import path exists (removes the "every Frozen golden declares its own struct" friction; requires relocating the builtin Frozen DefId assignment from Phase 1b to after Phase 3a stdlib-module loading). (d-full) Auto-impl Freeze for user structs with all-Freeze fields so users don't have to write `impl Freeze for MyStruct {}` boilerplate — the structural walk is already in `type_is_freeze` but it only fires as a fallback when no manual impl exists; expanding means lifting that into a primary path. (b-followup) User-defined enum variants with generic payloads — requires threading `TypeArgs` through `VariantLayout.fields.hir_type` so that `Frozen<MyResult<String, i32>>` is walked correctly.
