...
|------|----------|---------|------------|--------|
| 2026-04-07 | codegen | Nested closure inside another closure's body: inner closure function definition not emitted. `llc-18: use of undefined value '@blood_closure_<id>'`. Simple closures and captured closures work. | Suspected: codegen worklist collection does not walk closure bodies to enqueue nested closures. Location: `codegen.blood` worklist + `mir_lower_expr.blood:collect_closure_captures`. | Open. Reproducer: `tests/golden/t04_nested_closure_xfail.blood`. Tracked as S-7 in audit. |
| 2026-04-07 | typeck | Compiler silently accepts function calls with wrong arity; downstream codegen emits invalid LLVM IR (observed: 3-arg call to 2-arg function produced `trunc i32 to i64` cast). | Typeck does not validate arity in function-call paths. | Open. Tracked as NEW-4 in audit. First observed surfacing NEW-1 (runtime build failure). |
