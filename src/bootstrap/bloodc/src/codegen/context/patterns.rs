...
impl<'ctx, 'a> CodegenContext<'ctx, 'a> {
    ...
    pub(super) fn compile_match(
        ...
        for (i, _) in arms.iter().enumerate() {
            let test_bb = self
                .context
                .append_basic_block(fn_value, &format!("match.arm{}.test", i));
            let body_bb = self
                .context
                .append_basic_block(fn_value, &format!("match.arm{}.body", i));
        ...
        let unreachable_bb = self
            .context
            .append_basic_block(fn_value, "match.unreachable");
        ...
        for (i, arm) in arms.iter().enumerate() {
            ...
            let final_match = if let Some(guard) = &arm.guard {
                ...
                let guard_bb = self
                    .context
                    .append_basic_block(fn_value, &format!("match.arm{}.guard", i));
                self.builder
                    .build_conditional_branch(matches, guard_bb, next_test)
                    .map_err(|e| {
                        vec![Diagnostic::error(
                            format!("LLVM error: {}", e),
                            self.current_span(),
                ...
                self.builder
                    .build_conditional_branch(matches, body_bb, next_test)
                    .map_err(|e| {
                        vec![Diagnostic::error(
                            format!("LLVM error: {}", e),
                            self.current_span(),
            ...
            self.builder
                .build_conditional_branch(final_match, body_bb, next_test)
                .map_err(|e| {
                    vec![Diagnostic::error(
                        format!("LLVM error: {}", e),
                        self.current_span(),
        ...
        if !result_ty.is_unit() && !incoming.is_empty() {
            let phi = self
                .builder
                .build_phi(self.lower_type(result_ty), "match.result")
                .map_err(|e| {
                    vec![Diagnostic::error(
                        format!("LLVM error: {}", e),
                        self.current_span(),
    ...
    pub(super) fn compile_pattern_test(
        ...
        match &pattern.kind {
            ...
            hir::PatternKind::Variant {
                ...
                let (discriminant, struct_val_opt) = match scrutinee {
                    ...
                    _ => {
                        return Err(vec![Diagnostic::error(
                            format!(
                                "Expected enum value in variant pattern, found {:?}",
                                scrutinee.get_type()
                            ),
                            pattern.span,
                        )]);
                ...
                if !fields.is_empty() {
                    let struct_val = struct_val_opt.ok_or_else(|| {
                        vec![Diagnostic::error(
            ...
            hir::PatternKind::Slice {
                ...
                if let BasicValueEnum::ArrayValue(arr) = scrutinee {
                    ...
                    let array_size = match pattern.ty.kind() {
                        hir::TypeKind::Array { size, .. } => size.as_u64().ok_or_else(|| {
                            vec![Diagnostic::error(
    ...
    pub(super) fn compile_value_eq(
        ...
        match (a, b) {
            ...
            (BasicValueEnum::PointerValue(a), BasicValueEnum::PointerValue(b)) => self
                .builder
                .build_int_compare(
                    IntPredicate::EQ,
                    self.builder
                        .build_ptr_to_int(*a, self.context.i64_type(), "ptr_a")
                        .map_err(|e| {
                            vec![Diagnostic::error(
                                format!("LLVM error: {}", e),
                                self.current_span(),
                    ...
                    self.builder
                        .build_ptr_to_int(*b, self.context.i64_type(), "ptr_b")
                        .map_err(|e| {
                            vec![Diagnostic::error(
                                format!("LLVM error: {}", e),
                                self.current_span(),
    ...
    pub(super) fn compile_pattern_bindings(
        ...
        match &pattern.kind {
            ...
            hir::PatternKind::Ref { inner, .. } => self.compile_pattern_bindings(inner, scrutinee),
            hir::PatternKind::Slice {
                prefix,
                slice,
                suffix,
                            ...
                            hir::PatternKind::Binding { local_id, .. } => {
                                ...
                                if rest_len > 0 {
                                    ...
                                    for i in 0..rest_len {
                                        ...
                                        self.builder.build_store(dest_ptr, elem).map_err(|e| {
                                            vec![Diagnostic::error(
                                                format!("LLVM error: {}", e),
                                                slice_pat.span,
                                            )]
