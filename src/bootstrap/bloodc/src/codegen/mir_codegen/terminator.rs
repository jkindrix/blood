...
use crate::mir::types::{
    BasicBlockId, Constant, ConstantKind, Operand, Place, Terminator, TerminatorKind,
};
    ...
    fn compile_mir_terminator(
        ...
        match &term.kind {
            TerminatorKind::Goto { target } => {
                let target_bb = llvm_blocks.get(target).ok_or_else(|| {
                    vec![Diagnostic::error(
                        format!("Target block {} not found", target),
                        term.span,
                    )]
                })?;
                self.builder
            ...
            TerminatorKind::SwitchInt { discr, targets } => {
                ...
                self.builder
                    .build_switch(discr_int, otherwise_bb, &cases)
                    .map_err(|e| {
                        vec![Diagnostic::error(
                            format!("LLVM switch error: {}", e),
                            term.span,
                        )]
                    })?;
            }
            ...
            TerminatorKind::Return => {
                ...
                let is_main =
                    self.current_fn_def_id == self.main_fn_def_id && self.main_fn_def_id.is_some();
                ...
                if let Some(&ret_ptr) = self.locals.get(&ret_local) {
                    ...
                    if !ret_ty.is_unit() {
                        ...
                        let ret_val = self
                            .builder
                            .build_load(ret_llvm_ty, ret_ptr, "ret_val")
                            .map_err(|e| {
                                vec![Diagnostic::error(
            ...
            TerminatorKind::Assert {
                ...
                self.compile_assert_terminator(
                    cond,
                    *expected,
                    msg,
                    target,
            ...
            TerminatorKind::DropAndReplace {
                ...
                let store_inst = self.builder.build_store(ptr, new_val).map_err(|e| {
                    vec![Diagnostic::error(
                ...
                let target_bb = llvm_blocks.get(target).ok_or_else(|| {
                    vec![Diagnostic::error(
                        "DropAndReplace target not found",
                        term.span,
                    )]
                })?;
                self.builder
...
impl<'ctx, 'a> CodegenContext<'ctx, 'a> {
    ...
    fn compile_call_terminator(
        ...
        let expected_param_types: Vec<crate::hir::Type> = match func {
            Operand::Constant(Constant {
                kind: ConstantKind::FnDef(_def_id),
                ty,
            }) => {
        ...
        let call_result = match func {
            Operand::Constant(Constant {
                kind: ConstantKind::FnDef(def_id),
                ty,
            }) => {
                ...
                if let Some(&fn_value) = self.functions.get(def_id) {
                    ...
                    let mut converted_args: Vec<BasicMetadataValueEnum> =
                        ...
                        let converted = if let Some(param_type) = param_types.get(i) {
                            if val.is_int_value() && param_type.is_int_type() {
                                ...
                                if is_gen_ref {
                                    ...
                                    self.builder
                                        .build_extract_value(sv, 0, "gen_ref_strip")
                                        .map(|v| v.into())
                                        .unwrap_or((*val).into())
                                } else {
                                    ...
                                    let natural_alignment =
                                        self.get_type_alignment_for_value(struct_val.into());
                                ...
                                if ptr_val.get_type() != expected_ptr_type {
                                    self.builder
                                        .build_pointer_cast(ptr_val, expected_ptr_type, "ptr_cast")
                                        .map(|p| p.into())
                                        .unwrap_or((*val).into())
                                } else {
                                    (*val).into()
                                ...
                                if alloca.get_type() != expected_ptr_type {
                                    self.builder
                                        .build_pointer_cast(alloca, expected_ptr_type, "ptr_cast")
                                        .map(|p| p.into())
                                        .unwrap_or(alloca.into())
                                } else {
                                    alloca.into()
                                ...
                                // Set alignment on alloca (request full alignment)
                                let natural_alignment =
                                    self.get_type_alignment_for_value(array_val.into());
                                if let Some(inst) = alloca.as_instruction() {
                                    let _ = inst.set_alignment(natural_alignment);
                                }
                                let store_inst =
                                ...
                                if sv.get_type() != expected_struct
                                    && sv.get_type().count_fields() == 2
                                {
                                    if let Ok(ptr_val) =
                                        self.builder.build_extract_value(sv, 0, "genref_deref")
                                    {
                    ...
                    self.builder
                        .build_call(fn_value, &converted_args, "call_result")
                        .map_err(|e| {
                            vec![Diagnostic::error(format!("LLVM call error: {}", e), span)]
                        })?
                    ...
                    if is_ptr_read {
                        ...
                        let elem_type: inkwell::types::BasicTypeEnum = match builtin_name.as_str() {
                            ...
                            _ => {
                                return Err(vec![Diagnostic::error(
                                    format!("Unknown ptr_read variant: {}", builtin_name),
                                    span,
                                )])
                            }
                        };
                        ...
                        let typed_ptr = self
                            .builder
                            .build_int_to_ptr(ptr_arg, ptr_type, "ptr_cast")
                            .map_err(|e| {
                                vec![Diagnostic::error(
                                    format!("LLVM int_to_ptr error: {}", e),
                                    span,
                                )]
                            })?;
                        ...
                        let loaded_val = self
                            .builder
                            .build_load(elem_type, typed_ptr, "ptr_load")
                            .map_err(|e| {
                                vec![Diagnostic::error(format!("LLVM load error: {}", e), span)]
                            })?;
                        ...
                        let store_inst =
                            self.builder
                                .build_store(dest_ptr, loaded_val)
                                .map_err(|e| {
                                    vec![Diagnostic::error(
                                        format!("LLVM store error: {}", e),
                                        span,
                    ...
                    if is_ptr_write {
                        ...
                        if arg_vals.len() < 2 {
                            return Err(vec![Diagnostic::error(
                                "ptr_write requires pointer and value arguments".to_string(),
                                span,
                            )]);
                        }
                        ...
                        let _elem_type: inkwell::types::BasicTypeEnum = match builtin_name.as_str()
                        {
                            ...
                            _ => {
                                return Err(vec![Diagnostic::error(
                                    format!("Unknown ptr_write variant: {}", builtin_name),
                                    span,
                                )])
                            }
                        };
                        ...
                        self.builder
                            .build_store(typed_ptr, value_arg)
                            .map_err(|e| {
                                vec![Diagnostic::error(format!("LLVM store error: {}", e), span)]
                            })?;
                    ...
                    if builtin_name == "persist" {
                        if let Some(arg_val) = arg_vals.first() {
                            let dest_ptr =
                                self.compile_mir_place(destination, body, escape_results)?;
                            let store_inst =
                                self.builder.build_store(dest_ptr, *arg_val).map_err(|e| {
                                    vec![Diagnostic::error(
                                        format!("LLVM store error in persist: {}", e),
                                        span,
                                    )]
                                })?;
                    ...
                    if let Some(intrinsic_name) = intrinsic_name {
                        ...
                        let arg_type = if let Some(arg) = arg_vals.first() {
                            ...
                            return Err(vec![Diagnostic::error(
                                format!(
                                    "Math intrinsic {} requires at least one argument",
                                    builtin_name
                                ),
                                span,
                            )]);
                        ...
                        let intrinsic_fn = intrinsic
                            ...
                            .ok_or_else(|| {
                                vec![Diagnostic::error(
                                    format!(
                                        "Could not get declaration for {} intrinsic",
                                        intrinsic_name
                                    ),
                                    span,
                                )]
                        ...
                        self.builder
                            .build_call(intrinsic_fn, &arg_metas, "math_intrinsic")
                            .map_err(|e| {
                                vec![Diagnostic::error(
                                    format!("LLVM intrinsic call error: {}", e),
                                    span,
                                )]
                            })?
                        ...
                        let mut output_buffer_alloca: Option<(
                        ...
                        for (i, val) in arg_vals.iter().enumerate() {
                            let converted = if let Some(param_type) = param_types.get(i) {
                                if val.is_int_value() && param_type.is_int_type() {
                                    ...
                                    // Set alignment on alloca
                                    let alignment =
                                        self.get_type_alignment_for_value(int_val.into());
                                    if let Some(inst) = alloca.as_instruction() {
                                        let _ = inst.set_alignment(alignment);
                                    }
                                    let store_inst =
                        ...
                        if needs_elem_size && !args.is_empty() {
                            ...
                            let elem_size: u64 = match builtin_name.as_str() {
                                ...
                                "vec_pop" | "vec_reverse" | "vec_get" | "vec_get_ptr"
                                | "vec_free" | "vec_first" | "vec_last" => {
                                ...
                                "option_unwrap" | "option_try" | "option_expect"
                                | "option_unwrap_or" | "option_ok_or" | "option_as_ref"
                                | "option_as_mut" | "option_take" => {
                            ...
                            if matches!(
                                ...
                                if matches!(
                                    builtin_name.as_str(),
                                    "option_expect" | "result_expect" | "result_expect_err"
                                ) {
                                ...
                                if matches!(
                                    ...
                                    if args.len() >= 2 {
                                        ...
                                        if default_val.is_int_value() {
                                            ...
                                            self.builder.build_store(alloca, int_val).map_err(
                                                |e| {
                                                    vec![Diagnostic::error(
                                                        format!("LLVM store error: {}", e),
                                                        span,
                                            ...
                                            self.builder.build_store(alloca, struct_val).map_err(
                                                |e| {
                                                    vec![Diagnostic::error(
                                                        format!("LLVM store error: {}", e),
                                                        span,
                                ...
                                if builtin_name.as_str() == "option_ok_or" {
                                    ...
                                    if args.len() >= 2 {
                                        ...
                                        if err_val.is_int_value() {
                                            ...
                                            // Add error size
                                            let err_size = self
                                                .get_type_size_in_bytes(int_val.get_type().into());
                                            converted_args.push(
                                                self.context
                                                    .i64_type()
                                                    .const_int(err_size, false)
                                                    .into(),
                                            );
                                            ...
                                            converted_args.push(ptr.into());
                                            // Add error size
                                            let err_size = self.get_type_size_in_bytes(
                                                struct_val.get_type().into(),
                                            );
                                            converted_args.push(
                                                self.context
                                                    .i64_type()
                                ...
                                if builtin_name.as_str() == "result_and" {
                                    ...
                                    let err_size = if let crate::hir::TypeKind::Adt {
                                        args: type_args,
                                        ..
                                    } = res_inner.kind()
                                    {
                                    ...
                                    converted_args.push(
                                        self.context.i64_type().const_int(err_size, false).into(),
                                    );
                                }
                                ...
                                let payload_llvm_ty = if let crate::hir::TypeKind::Adt {
                                    args: type_args,
                                    ..
                                } = inner_ty.kind()
                                {
                                    match builtin_name.as_str() {
                                        ...
                                        "option_unwrap" | "option_try" | "option_expect"
                                        | "option_unwrap_or" | "result_unwrap" | "result_try"
                                        | "result_expect" | "result_unwrap_or" => {
                                            if let Some(payload_ty) = type_args.first() {
                                        ...
                                        "option_and" => {
                                            ...
                                            if args.len() >= 2 {
                                                let other_ty =
                                                    self.get_operand_type(&args[1], body);
                                                let other_inner = match other_ty.kind() {
                                                    crate::hir::TypeKind::Ref { inner, .. } => {
                                                        inner.clone()
                                                    }
                                        ...
                                        "option_or" | "option_xor" | "option_take"
                                        | "option_replace" => {
                                            // Output is Option<T>
                                            if let Some(payload_ty) = type_args.first() {
                                                ...
                                                self.context
                                                    .struct_type(
                                                        &[self.context.i32_type().into(), t_llvm],
                                                        false,
                                                    )
                                                    .into()
                                            } else {
                        ...
                        if builtin_name.as_str() == "vec_new" {
                            ...
                            let out_alloca =
                                self.builder.build_alloca(vec_ty, "vec_out").map_err(|e| {
                                    vec![Diagnostic::error(
                                        format!("LLVM alloca error: {}", e),
                                        span,
                                    )]
                            ...
                            let elem_size: u64 = if let crate::hir::TypeKind::Adt {
                                args: type_args,
                                ..
                            } = dest_ty.kind()
                            {
                        ...
                        let call_instr = self
                            .builder
                            .build_call(fn_value, &converted_args, "builtin_call")
                            .map_err(|e| {
                                vec![Diagnostic::error(format!("LLVM call error: {}", e), span)]
                            })?;
                        ...
                        if let Some((out_alloca, out_ty)) = output_buffer_alloca {
                            ...
                            let result = self
                                .builder
                                .build_load(out_ty, out_alloca, "out_result")
                                .map_err(|e| {
                                    vec![Diagnostic::error(format!("LLVM load error: {}", e), span)]
                                })?;
                            // Convert result to match destination type if needed
                            let converted_result =
                                self.convert_value_for_store(result, dest_llvm_ty, span)?;
                            let store_inst = self
                                .builder
                                .build_store(dest_ptr, converted_result)
                                .map_err(|e| {
                                vec![Diagnostic::error(format!("LLVM store error: {}", e), span)]
                            })?;
                    ...
                    } = ty.kind()
                    {
                        ...
                        let concrete_params: Vec<_> = if args.len() == params.len() {
                            args.iter()
                                .map(|arg| self.get_operand_type(arg, body))
                                .collect()
                        } else {
                            params.clone()
                        };
                        ...
                        let concrete_ret = self
                            .get_place_base_type(destination, body)
                            .unwrap_or_else(|_| ret.clone());
                        // Attempt monomorphization, passing explicit const args from turbofish
                        if let Some(mono_fn) = self.monomorphize_function_with_const_args(
                            *def_id,
                            &concrete_params,
                            &concrete_ret,
                            const_args,
                            ...
                            self.builder
                                .build_call(mono_fn, &arg_metas, "mono_call")
                                .map_err(|e| {
                                    vec![Diagnostic::error(format!("LLVM call error: {}", e), span)]
                                })?
                        } else {
                        ...
                        return Err(vec![Diagnostic::error(
                            format!(
                                "Generic function {:?} called with non-function type: {:?}",
                                def_id, ty
                            ),
                            span,
                        )]);
                ...
                if let Some(closure_def_id) = get_closure_def_id(place, body) {
                    ...
                    if let Some(&fn_value) = self.functions.get(&closure_def_id) {
                        ...
                        let full_args: Vec<BasicMetadataValueEnum> = if has_env_param {
                            ...
                            let closure_val =
                                self.compile_mir_operand(func, body, escape_results)?;
                            ...
                            let is_inline =
                                self.should_inline_closure_env(&closure_def_id, place.as_local());
                            ...
                            let env_ptr = if is_inline {
                                ...
                                let sv = if let BasicValueEnum::StructValue(sv) = closure_val {
                                    ...
                                    self.builder
                                        .build_load(closure_llvm_ty, ptr, "closure.load")
                                        .map_err(|e| {
                                            vec![Diagnostic::error(
                                                format!("LLVM load error: {}", e),
                                                span,
                                ...
                                if num_captures > 0 {
                                    ...
                                    let mut capture_types =
                                        Vec::with_capacity(num_captures as usize);
                                    let mut capture_vals =
                                        Vec::with_capacity(num_captures as usize);
                                    ...
                                    let captures_ty =
                                        self.context.struct_type(&capture_types, false);
                                    ...
                                    let env_alloca = self
                                        .builder
                                        .build_alloca(captures_ty, "inline_env")
                                        .map_err(|e| {
                                            vec![Diagnostic::error(
                                                format!("LLVM alloca error: {}", e),
                                                span,
                                    ...
                                    self.builder.build_store(env_alloca, captures_val).map_err(
                                        |e| {
                                            vec![Diagnostic::error(
                                                format!("LLVM store error: {}", e),
                                                span,
                                    ...
                                    self.builder
                                        .build_pointer_cast(env_alloca, i8_ptr_ty, "inline_env_ptr")
                                        .map_err(|e| {
                                            vec![Diagnostic::error(
                                                format!("LLVM pointer cast error: {}", e),
                                                span,
                            ...
                            for (i, val) in arg_vals.iter().enumerate() {
                                if let Some(param_type) = param_types.get(i + 1) {
                                    if let Ok(basic_ty) = (*param_type).try_into() {
                                        full_args.push(
                                            self.convert_arg_for_param(*val, basic_ty, span)?,
                                        );
                                    } else {
                        ...
                        self.builder
                            .build_call(fn_value, &full_args, "closure_call")
                            .map_err(|e| {
                                vec![Diagnostic::error(format!("LLVM call error: {}", e), span)]
                            })?
                    } else {
                        return Err(vec![Diagnostic::error(
                            format!("Closure function {:?} not found", closure_def_id),
                            span,
                        )]);
                    }
                } else {
                    ...
                    let (fn_ptr, env_ptr) = if let BasicValueEnum::StructValue(sv) = func_val {
                        ...
                        let loaded = self
                            .builder
                            .build_load(func_llvm_ty, ptr, "fn.load")
                            .map_err(|e| {
                                vec![Diagnostic::error(format!("LLVM load error: {}", e), span)]
                            })?;
                        let sv = loaded.into_struct_value();
                        ...
                        return Err(vec![Diagnostic::error(
                            format!(
                                "Expected fn struct or pointer for call, got {:?}",
                                func_val.get_type()
                            ),
                            span,
                        )]);
                    ...
                    let (fn_type, llvm_param_types) =
                        if let crate::hir::TypeKind::Fn { params, ret, .. } = func_ty.kind() {
                            let mut param_types: Vec<BasicMetadataTypeEnum> =
                                Vec::with_capacity(params.len() + 1);
                            let mut llvm_params: Vec<inkwell::types::BasicTypeEnum> =
                                Vec::with_capacity(params.len() + 1);
                    ...
                    let mut full_args: Vec<BasicMetadataValueEnum> =
                    ...
                    self.builder
                        .build_indirect_call(fn_type, fn_ptr, &full_args, "call_result")
                        .map_err(|e| {
                            vec![Diagnostic::error(format!("LLVM call error: {}", e), span)]
                        })?
                }
        ...
        if let Some(ret_val) = call_result.try_as_basic_value().basic() {
            ...
            let store_inst = self
                .builder
                .build_store(dest_ptr, converted_val)
        ...
        if let Some(target_bb_id) = target {
            let target_bb = llvm_blocks
                .get(target_bb_id)
                .ok_or_else(|| vec![Diagnostic::error("Call target block not found", span)])?;
            self.builder
                .build_unconditional_branch(*target_bb)
    ...
    fn compile_assert_terminator(
        ...
        let check = self
            .builder
            .build_int_compare(IntPredicate::EQ, cond_int, expected_int, "assert_check")
            .map_err(|e| {
                vec![Diagnostic::error(
                    format!("LLVM compare error: {}", e),
                    span,
                )]
            })?;
        ...
        let current_fn = self
            .current_fn
            .ok_or_else(|| vec![Diagnostic::error("No current function", span)])?;
        ...
        self.builder
            .build_conditional_branch(check, assert_ok_bb, assert_fail_bb)
            .map_err(|e| vec![Diagnostic::error(format!("LLVM branch error: {}", e), span)])?;
        ...
        let msg_global = self
            ...
            .map_err(|e| {
                vec![Diagnostic::error(
                    format!("LLVM global string error: {}", e),
                    span,
                )]
            })?;
        ...
        let target_bb = llvm_blocks
            .get(target)
            .ok_or_else(|| vec![Diagnostic::error("Assert target block not found", span)])?;
        self.builder
            .build_unconditional_branch(*target_bb)
            .map_err(|e| vec![Diagnostic::error(format!("LLVM branch error: {}", e), span)])?;
    ...
    fn compile_perform_terminator(
        ...
        let take_snapshot_fn = self
            .module
            .get_function("blood_effect_context_take_snapshot")
            .ok_or_else(|| {
                vec![Diagnostic::error(
                    "Runtime function blood_effect_context_take_snapshot not found",
                    span,
                )]
            })?;
        let saved_snapshot = self
            .builder
            .build_call(take_snapshot_fn, &[], "saved_snap")
            .map_err(|e| vec![Diagnostic::error(format!("LLVM call error: {}", e), span)])?
            .try_as_basic_value()
            .basic()
        ...
        let snapshot_create = self
            .module
            .get_function("blood_snapshot_create")
            .ok_or_else(|| {
                vec![Diagnostic::error(
        ...
        let snapshot = self
            .builder
            .build_call(snapshot_create, &[], "snapshot")
            .map_err(|e| vec![Diagnostic::error(format!("LLVM call error: {}", e), span)])?
            .try_as_basic_value()
            .basic()
        ...
        let snapshot_add_entry = self
            .module
            .get_function("blood_snapshot_add_entry")
            .ok_or_else(|| {
                vec![Diagnostic::error(
        ...
        if let Some(escape) = escape_results {
            for local in &body.locals {
                ...
                if escape.needs_effect_snapshot(local.id)
                    && super::types::type_may_contain_genref_impl(&local.ty)
                {
                    ...
                    if let Some(&local_ptr) = self.locals.get(&local.id) {
                        ...
                        if ptr_val.is_pointer_value() {
                            let address = self
                                .builder
                                .build_ptr_to_int(ptr_val.into_pointer_value(), i64_ty, "addr")
                                .map_err(|e| {
                                    vec![Diagnostic::error(
                            ...
                            let generation =
                                self.get_generation_for_address(address, i32_ty, span)?;
                            ...
                            if bit_width == 128 {
                                ...
                                let address = self
                                    ...
                                    .map_err(|e| {
                                        vec![Diagnostic::error(
                                            format!("LLVM shift error: {}", e),
                                            span,
                                        )]
                                    })?;
                                let address = self
                                    .builder
                                    .build_int_truncate(address, i64_ty, "addr")
                                    .map_err(|e| {
                                        vec![Diagnostic::error(
                                            format!("LLVM truncate error: {}", e),
                                            span,
                                        )]
                                    })?;

                                // Extract generation from bits 32-63 (next 32 bits after metadata)
                                let gen_shifted = self
                                    .builder
                                    .build_right_shift(
                                        int_val,
                                ...
                                let generation = self
                                    .builder
                                    .build_int_truncate(gen_shifted, i32_ty, "gen")
                                    .map_err(|e| {
                                        vec![Diagnostic::error(
                                            format!("LLVM truncate error: {}", e),
                                            span,
                                ...
                                let address = if bit_width < 64 {
                                    self.builder
                                        .build_int_z_extend(int_val, i64_ty, "addr")
                                        .map_err(|e| {
                                            vec![Diagnostic::error(
                                                format!("LLVM extend error: {}", e),
                                                span,
                                            )]
                                        })?
                                } else {
        ...
        let set_snapshot_fn = self
            .module
            .get_function("blood_effect_context_set_snapshot")
            .ok_or_else(|| {
                vec![Diagnostic::error(
                    "Runtime function blood_effect_context_set_snapshot not found",
                    span,
                )]
            })?;
        self.builder
            .build_call(set_snapshot_fn, &[snapshot.into()], "")
            .map_err(|e| vec![Diagnostic::error(format!("LLVM call error: {}", e), span)])?;

        // Call blood_perform with effect_id, op_index, args
        let perform_fn = self.module.get_function("blood_perform").ok_or_else(|| {
            vec![Diagnostic::error(
                "Runtime function blood_perform not found",
                span,
            )]
        })?;
        ...
        let args_ptr = if arg_vals.is_empty() {
            ...
            let args_alloca = self
                .builder
                .build_alloca(array_ty, "perform_args")
            ...
            for (i, val) in arg_vals.iter().enumerate() {
                ...
                let val_i64 = match *val {
                    ...
                    BasicValueEnum::FloatValue(fv) => self
                        .builder
                        .build_bit_cast(fv, i64_ty, "float_as_i64")
                        .map_err(|e| {
                            vec![Diagnostic::error(
                                format!("LLVM bitcast error: {}", e),
                                span,
                            )]
                        })?
                        .into_int_value(),
                    BasicValueEnum::PointerValue(pv) => self
                        .builder
                        .build_ptr_to_int(pv, i64_ty, "ptr_as_i64")
                        .map_err(|e| {
                            vec![Diagnostic::error(
                                format!("LLVM ptr_to_int error: {}", e),
                                span,
                    ...
                    BasicValueEnum::StructValue(sv) => {
                        ...
                        let struct_alloca = self
                            .builder
                            .build_alloca(sv.get_type(), &format!("struct_arg_{}", i))
                            .map_err(|e| {
                                vec![Diagnostic::error(format!("LLVM alloca error: {}", e), span)]
                            })?;
                        self.builder.build_store(struct_alloca, sv).map_err(|e| {
                            vec![Diagnostic::error(format!("LLVM store error: {}", e), span)]
                        })?;
                        // Pass pointer as i64
                        self.builder
                            .build_ptr_to_int(struct_alloca, i64_ty, "struct_ptr_as_i64")
                            .map_err(|e| {
                                vec![Diagnostic::error(
                                    format!("LLVM ptr_to_int error: {}", e),
                                    span,
                    ...
                    BasicValueEnum::ArrayValue(av) => {
                        ...
                        let array_alloca = self
                            .builder
                            .build_alloca(av.get_type(), &format!("array_arg_{}", i))
                            .map_err(|e| {
                                vec![Diagnostic::error(format!("LLVM alloca error: {}", e), span)]
                            })?;
                        self.builder.build_store(array_alloca, av).map_err(|e| {
                            vec![Diagnostic::error(format!("LLVM store error: {}", e), span)]
                        })?;
                        // Pass pointer as i64
                        self.builder
                            .build_ptr_to_int(array_alloca, i64_ty, "array_ptr_as_i64")
                            .map_err(|e| {
                                vec![Diagnostic::error(
                                    format!("LLVM ptr_to_int error: {}", e),
                                    span,
                ...
                self.builder.build_store(gep, val_i64).map_err(|e| {
                    vec![Diagnostic::error(format!("LLVM store error: {}", e), span)]
                })?;
            }
        ...
        if needs_region_suspension {
            let add_suspended_fn = self
                .module
                .get_function("blood_continuation_add_suspended_region")
                .ok_or_else(|| {
                    vec![Diagnostic::error(
                        "Runtime function blood_continuation_add_suspended_region not found",
                        span,
                    )]
                })?;
        ...
        if !is_unit_type {
            let dest_ptr = self.compile_mir_place(destination, body, escape_results)?;
            let result_val = result.try_as_basic_value().basic().ok_or_else(|| {
                vec![ice_err!(
                    span,
            ...
            let converted_result: BasicValueEnum = if dest_llvm_ty.is_int_type() {
                ...
                if dest_bits < 64 {
                    ...
                    self.builder
                        .build_int_truncate(result_i64, dest_int_ty, "perform_trunc")
                        .map_err(|e| {
                            vec![Diagnostic::error(
                                format!("LLVM truncate error: {}", e),
                                span,
                    ...
                    self.builder
                        .build_int_z_extend(result_i64, dest_int_ty, "perform_ext")
                        .map_err(|e| {
                            vec![Diagnostic::error(format!("LLVM extend error: {}", e), span)]
                        })?
                        .into()
                } else {
                ...
                let result_i64 = result_val.into_int_value();
                self.builder
                    .build_int_to_ptr(result_i64, dest_llvm_ty.into_pointer_type(), "perform_ptr")
                    .map_err(|e| {
                        vec![Diagnostic::error(
                            format!("LLVM int_to_ptr error: {}", e),
                            span,
                ...
                let temp_alloca =
                    self.builder
                        .build_alloca(i64_ty, "perform_temp")
                        .map_err(|e| {
                            vec![Diagnostic::error(format!("LLVM alloca error: {}", e), span)]
                        })?;
                ...
                self.builder
                    .build_store(temp_alloca, result_i64)
                    .map_err(|e| {
                        vec![Diagnostic::error(format!("LLVM store error: {}", e), span)]
                    })?;
                ...
                self.builder
                    .build_load(dest_struct_ty, temp_alloca, "perform_struct")
        ...
        let snapshot_validate = self
            .module
            .get_function("blood_snapshot_validate")
            .ok_or_else(|| {
                vec![Diagnostic::error(
        ...
        let validation_result = self
            .builder
            .build_call(snapshot_validate, &[snapshot.into()], "validation")
            .map_err(|e| vec![Diagnostic::error(format!("LLVM call error: {}", e), span)])?
            .try_as_basic_value()
            .basic()
        ...
        let snapshot_destroy = self
            .module
            .get_function("blood_snapshot_destroy")
            .ok_or_else(|| {
                vec![Diagnostic::error(
        ...
        self.builder
            .build_call(snapshot_destroy, &[snapshot.into()], "")
            .map_err(|e| vec![Diagnostic::error(format!("LLVM call error: {}", e), span)])?;
        ...
        self.builder
            .build_conditional_branch(is_valid, continue_bb, stale_bb)
            .map_err(|e| vec![Diagnostic::error(format!("LLVM branch error: {}", e), span)])?;
        ...
        self.builder
            .build_call(set_snapshot_fn, &[saved_snapshot.into()], "")
            .map_err(|e| vec![Diagnostic::error(format!("LLVM call error: {}", e), span)])?;

        let target_bb = llvm_blocks
            .get(target)
            .ok_or_else(|| vec![Diagnostic::error("Perform target not found", span)])?;
        self.builder
            .build_unconditional_branch(*target_bb)
            .map_err(|e| vec![Diagnostic::error(format!("LLVM branch error: {}", e), span)])?;
    ...
    fn compile_resume_terminator(
        ...
        let fn_value = self
            .current_fn
            .ok_or_else(|| vec![Diagnostic::error("No current function for Resume", span)])?;
        ...
        if let Some(val_op) = value {
            ...
            if let Some(&ret_ptr) = self.locals.get(&LocalId::new(0)) {
                self.builder.build_store(ret_ptr, val).map_err(|e| {
                    vec![Diagnostic::error(format!("LLVM store error: {}", e), span)]
                })?;
            }
        }
        ...
        let get_snapshot_fn = self
            .module
            .get_function("blood_effect_context_get_snapshot")
            .ok_or_else(|| {
                vec![Diagnostic::error(
                    "Runtime function blood_effect_context_get_snapshot not found",
                    span,
                )]
            })?;
        ...
        let snapshot = self
            .builder
            .build_call(get_snapshot_fn, &[], "snapshot")
            .map_err(|e| vec![Diagnostic::error(format!("LLVM call error: {}", e), span)])?
            .try_as_basic_value()
            .basic()
            .ok_or_else(|| {
                vec![Diagnostic::error(
                    "blood_effect_context_get_snapshot returned void",
                    span,
                )]
            })?;
        ...
        self.builder
            .build_conditional_branch(snapshot_is_null, ok_bb, validate_bb)
            .map_err(|e| vec![Diagnostic::error(format!("LLVM branch error: {}", e), span)])?;
        ...
        let stale_index = self
            .builder
            .build_call(validate_fn, &[snapshot.into()], "stale_index")
            .map_err(|e| vec![Diagnostic::error(format!("LLVM call error: {}", e), span)])?
            .try_as_basic_value()
            .basic()
            .ok_or_else(|| {
                vec![Diagnostic::error(
                    "blood_snapshot_validate returned void",
                    span,
                )]
            })?;
        ...
        self.builder
            .build_conditional_branch(is_valid, ok_bb, stale_bb)
            .map_err(|e| vec![Diagnostic::error(format!("LLVM branch error: {}", e), span)])?;
        ...
        let panic_fn = self
            .module
            .get_function("blood_snapshot_stale_panic")
            .ok_or_else(|| {
                vec![Diagnostic::error(
                    "Runtime function blood_snapshot_stale_panic not found",
                    span,
                )]
            })?;

        self.builder
            .build_call(panic_fn, &[snapshot.into(), stale_index.into()], "")
            .map_err(|e| vec![Diagnostic::error(format!("LLVM call error: {}", e), span)])?;
        ...
        self.builder
            .build_return(None)
            .map_err(|e| vec![Diagnostic::error(format!("LLVM return error: {}", e), span)])?;
    ...
    pub(super) fn convert_value_for_store(
        ...
        if val.is_int_value() && dest_type.is_int_type() {
            ...
            if val_width > dest_width {
                ...
                return Ok(self
                    .builder
                    .build_int_truncate(val_int, dest_int_type, "store_trunc")
                    .map(|v| v.into())
                    .unwrap_or(val));
                ...
                return Ok(self
                    .builder
                    .build_int_z_extend(val_int, dest_int_type, "store_zext")
                    .map(|v| v.into())
                    .unwrap_or(val));
            }
        }
        ...
        if val.is_int_value() && dest_type.is_pointer_type() {
            ...
            return Ok(self
                .builder
                .build_int_to_ptr(int_val, ptr_type, "store_int_to_ptr")
                .map(|p| p.into())
                .unwrap_or(val));
        }
        ...
        if val.is_pointer_value() && dest_type.is_int_type() {
            ...
            return Ok(self
                .builder
                .build_ptr_to_int(ptr_val, int_type, "store_ptr_to_int")
                .map(|i| i.into())
                .unwrap_or(val));
        }
        ...
        if val.is_float_value() && dest_type.is_float_type() {
            ...
            if float_val.get_type() != dest_float_type {
                return Ok(self
                    .builder
                    .build_float_cast(float_val, dest_float_type, "store_float_cast")
                    .map(|f| f.into())
                    .unwrap_or(val));
            }
        }
        ...
        if val.is_struct_value() && dest_type.is_struct_type() {
            ...
            let is_gen_ref_dest = dest_struct_type.count_fields() == 2
                ...
                && dest_struct_type
                    ...
                    .map_or(false, |t| {
                        t.is_int_type() && t.into_int_type().get_bit_width() == 32
                    });
            ...
            let val_is_gen_ref = {
                ...
                vt.count_fields() == 2
                    && vt
                        .get_field_type_at_index(0)
                        .map_or(false, |t| t.is_pointer_type())
                    && vt.get_field_type_at_index(1).map_or(false, |t| {
                        t.is_int_type() && t.into_int_type().get_bit_width() == 32
                    })
            };
            ...
            if is_gen_ref_dest && !val_is_gen_ref {
                ...
                fat_ref = self
                    .builder
                    .build_insert_value(fat_ref, alloca, 0, "auto_ref_ptr")
                    .map_err(|e| {
                        vec![Diagnostic::error(
                            format!("LLVM insert_value error: {}", e),
                            span,
                        )]
                    })?
                    .into_struct_value();
                ...
                fat_ref = self
                    .builder
                    .build_insert_value(fat_ref, gen_val, 1, "auto_ref_gen")
                    .map_err(|e| {
                        vec![Diagnostic::error(
                            format!("LLVM insert_value error: {}", e),
                            span,
            ...
            if val_struct_type.count_fields() == dest_struct_type.count_fields() {
                ...
                if needs_conversion {
                    ...
                    for i in 0..field_count {
                        let field_val = self
                            .builder
                            .build_extract_value(struct_val, i, &format!("field_{}", i))
                            .map_err(|e| {
                                vec![Diagnostic::error(
                                    format!("LLVM extract_value error: {}", e),
                                    span,
                                )]
                            })?;
                        ...
                        if let Some(dest_field_type) = dest_struct_type.get_field_type_at_index(i) {
                            ...
                            let converted_field = self.convert_value_for_store(
                                field_val.into(),
                                dest_field_type,
                                span,
                            )?;
        ...
        if val.is_pointer_value() && dest_type.is_struct_type() {
            ...
            if is_gen_ref_dest {
                ...
                fat_ref = self
                    .builder
                    .build_insert_value(fat_ref, ptr_val, 0, "promote_ptr")
                    .map_err(|e| {
                        vec![Diagnostic::error(
                            format!("LLVM insert_value error: {}", e),
                            span,
                ...
                fat_ref = self
                    .builder
                    .build_insert_value(fat_ref, gen_val, 1, "promote_gen")
                    .map_err(|e| {
                        vec![Diagnostic::error(
                            format!("LLVM insert_value error: {}", e),
                            span,
            ...
            let loaded = self
                .builder
                .build_load(dest_struct_ty, ptr_val, "store_ptr_load")
                .map_err(|e| vec![Diagnostic::error(format!("LLVM load error: {}", e), span)])?;
            return Ok(loaded);
        }
    ...
    fn compile_vtable_dispatch_call(
        ...
        let fat_ptr = arg_vals.first().ok_or_else(|| {
            vec![Diagnostic::error(
                "Vtable dispatch requires at least one argument (trait object)".to_string(),
                span,
            )]
        })?;
        ...
        let (trait_id, method_name) = self
            .trait_method_info
            .get(&method_def_id)
            .cloned()
            .ok_or_else(|| {
                vec![Diagnostic::error(
                    format!("Method {:?} not found in trait_method_info", method_def_id),
                    span,
                )]
            })?;

        let layout = self.vtable_layouts.get(&trait_id).ok_or_else(|| {
            vec![Diagnostic::error(
                format!("No vtable layout for trait {:?}", trait_id),
                span,
            )]
        })?;
        ...
        let slot_index = layout
            ...
            .ok_or_else(|| {
                vec![Diagnostic::error(
                    format!(
                        "Method '{}' not found in vtable layout for trait {:?}",
        ...
        let param_types: Vec<BasicMetadataTypeEnum> = call_args
            .iter()
        ...
        if let Some(target_block) = target {
            if let Some(llvm_block) = llvm_blocks.get(target_block) {
                self.builder
                    .build_unconditional_branch(*llvm_block)
                    .map_err(|e| {
                        vec![Diagnostic::error(format!("LLVM branch error: {}", e), span)]
                    })?;
            }
        }
