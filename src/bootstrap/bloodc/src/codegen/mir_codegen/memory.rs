...
impl<'ctx, 'a> MirMemoryCodegen<'ctx, 'a> for CodegenContext<'ctx, 'a> {
    fn get_generation_for_address(
        ...
        let get_gen_fn = self
            .module
            .get_function("blood_get_generation")
            .ok_or_else(|| {
                vec![ice_err!(
                    span,
        ...
        let gen_call_result = self
            .builder
            .build_call(get_gen_fn, &[address.into()], "gen_lookup")
...
pub(super) fn emit_generation_check_impl<'ctx, 'a>(
    ...
    let address = ctx
        .builder
        .build_ptr_to_int(ptr, i64_ty, "ptr_addr")
    ...
    let is_valid = ctx
        .builder
        .build_int_compare(IntPredicate::EQ, is_stale, zero, "is_valid")
        .map_err(|e| vec![Diagnostic::error(format!("LLVM error: {}", e), span)])?;

    ctx.builder
        .build_conditional_branch(is_valid, valid_bb, stale_bb)
...
pub(super) fn allocate_with_blood_alloc_impl<'ctx, 'a>(
    ...
    let gen_alloca = ctx
        .builder
        .build_alloca(i32_ty, &format!("_{}_gen", local_id.index))
    ...
    ctx.builder
        .build_store(gen_alloca, i32_ty.const_int(0, false))
    ...
    let address = ctx
        ...
        .ok_or_else(|| {
            vec![Diagnostic::error(
                "blood_alloc_or_abort returned void",
                span,
            )]
        })?
        .into_int_value();
...
pub(super) fn allocate_with_persistent_alloc_impl<'ctx, 'a>(
    ...
    let alloc_fn = ctx
        .module
        .get_function("blood_persistent_alloc")
        .unwrap_or_else(|| {
            ...
            ctx.module
                .add_function("blood_persistent_alloc", fn_type, None)
        });
    ...
    let slot_id_alloca = ctx
        .builder
        .build_alloca(i64_ty, &format!("_{}_slot_id", local_id.index))
        .map_err(|e| vec![Diagnostic::error(format!("LLVM alloca error: {}", e), span)])?;

    // Initialize slot ID to 0
    ctx.builder
        .build_store(slot_id_alloca, i64_ty.const_int(0, false))
        .map_err(|e| vec![Diagnostic::error(format!("LLVM store error: {}", e), span)])?;
    ...
    let raw_ptr = ctx
        ...
        .ok_or_else(|| {
            vec![Diagnostic::error(
                "blood_persistent_alloc returned void",
                span,
            )]
        })?
        .into_pointer_value();
...
pub(super) fn emit_stale_reference_perform<'ctx, 'a>(
    ...
    let perform_fn = ctx.module.get_function("blood_perform").ok_or_else(|| {
        vec![Diagnostic::error(
    ...
    let exp_i64 = ctx
        .builder
        .build_int_z_extend(expected_gen, i64_ty, "exp_i64")
        .map_err(|e| vec![Diagnostic::error(format!("LLVM extend error: {}", e), span)])?;
    let act_i64 = ctx
        .builder
        .build_int_z_extend(actual_gen, i64_ty, "act_i64")
        .map_err(|e| vec![Diagnostic::error(format!("LLVM extend error: {}", e), span)])?;
    ...
    let args_alloca = ctx
        .builder
        .build_alloca(array_ty, "stale_args")
        .map_err(|e| vec![Diagnostic::error(format!("LLVM alloca error: {}", e), span)])?;
    ...
    let gep0 = unsafe {
        ctx.builder
            .build_gep(array_ty, args_alloca, &[zero, zero], "stale_arg_0")
    }
    .map_err(|e| vec![Diagnostic::error(format!("LLVM GEP error: {}", e), span)])?;
    ctx.builder
        .build_store(gep0, exp_i64)
        .map_err(|e| vec![Diagnostic::error(format!("LLVM store error: {}", e), span)])?;
    let gep1 = unsafe {
        ctx.builder
            .build_gep(array_ty, args_alloca, &[zero, one], "stale_arg_1")
    }
    .map_err(|e| vec![Diagnostic::error(format!("LLVM GEP error: {}", e), span)])?;
    ctx.builder
        .build_store(gep1, act_i64)
        .map_err(|e| vec![Diagnostic::error(format!("LLVM store error: {}", e), span)])?;
