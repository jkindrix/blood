...
impl<'ctx, 'a> MirPlaceCodegen<'ctx, 'a> for CodegenContext<'ctx, 'a> {
    fn compile_mir_place(
        ...
        let is_closure_env = local_info_opt
            .map(|li| li.name.as_deref() == Some("__env"))
            .unwrap_or(false);
        let has_field_projections = place
            .projection
            .iter()
            .any(|p| matches!(p, PlaceElem::Field(_)));
        ...
        if is_closure_env && has_field_projections {
            ...
            let env_i8_ptr = self
                .builder
                .build_load(ptr_ty, current_ptr, "env_ptr")
                .map_err(|e| {
                    vec![Diagnostic::error(
        ...
        if std::env::var("BLOOD_DEBUG_PLACE").is_ok() {
            ...
            eprintln!(
                "[compile_mir_place] place: {:?}, base_ty: {:?}",
                place, base_ty
            );
            eprintln!("[compile_mir_place] projections: {:?}", place.projection);
            eprintln!(
                "[compile_mir_place] base_ptr type: {:?}",
                base_ptr.get_type().print_to_string()
            );
        }
        ...
        for (proj_idx, elem) in place.projection.iter().enumerate() {
            ...
            if std::env::var("BLOOD_DEBUG_PLACE").is_ok() {
                ...
                eprintln!(
                    "[compile_mir_place] current_ptr type: {:?}",
            ...
            current_ptr = match elem {
                PlaceElem::Deref => {
                    ...
                    let loaded = self
                        .builder
                        .build_load(load_ty, current_ptr, "deref")
                        .map_err(|e| {
                            vec![Diagnostic::error(
                                format!("LLVM load error: {}", e),
                                body.span,
                    ...
                    let ptr_val = match loaded {
                        ...
                        BasicValueEnum::StructValue(sv) => {
                            if is_fat_ptr {
                                ...
                                self.builder
                                    .build_extract_value(sv, 0, "fat_ptr_data")
                                    .map_err(|e| {
                                        vec![Diagnostic::error(
                                ...
                                let tmp_alloca = self
                                    .builder
                                    .build_alloca(struct_ty, "deref_tmp")
                                    .map_err(|e| {
                                        vec![Diagnostic::error(
                                ...
                                self.builder.build_store(tmp_alloca, sv).map_err(|e| {
                                    vec![Diagnostic::error(
                        ...
                        BasicValueEnum::IntValue(int_val) => {
                            ...
                            let ptr_ty = self.context.ptr_type(AddressSpace::default());
                            self.builder
                                .build_int_to_ptr(int_val, ptr_ty, "deref_int_to_ptr")
                                .map_err(|e| {
                                    vec![Diagnostic::error(
                    ...
                    let should_skip_gen_check = place.is_static()
                        || escape_results
                            .and_then(|er| {
                                place.as_local().map(|l| er.get(l) == EscapeState::NoEscape)
                            })
                            .unwrap_or(false);
                    ...
                    if false
                        && !should_skip_gen_check
                        && !self
                            .unchecked_checks
                            .contains(&crate::ast::UncheckedCheck::Generation)
                    {
                        if let Some(local_id) = place.as_local() {
                            if let Some(&gen_alloca) = self.local_generations.get(&local_id) {
                                ...
                                let expected_gen = self
                                    .builder
                                    .build_load(i32_ty, gen_alloca, "expected_gen")
                                    .map_err(|e| {
                                        vec![Diagnostic::error(
                                            format!("LLVM load error: {}", e),
                                            body.span,
                                ...
                                let local_storage_ptr = *self.locals.get(&local_id).expect(
                                    "ICE: local not found in locals map for generation check",
                                );
                                let address = self
                                    .builder
                                    .build_ptr_to_int(local_storage_ptr, i64_ty, "ptr_addr")
                                    .map_err(|e| {
                                        vec![Diagnostic::error(
                                ...
                                let validate_fn = self
                                    .module
                                    .get_function("blood_validate_generation")
                                    .ok_or_else(|| {
                                        vec![Diagnostic::error(
                                ...
                                let result = self
                                    ...
                                    .ok_or_else(|| {
                                        vec![Diagnostic::error(
                                            "blood_validate_generation returned void",
                                            body.span,
                                        )]
                                    })?
                                    .into_int_value();
                                ...
                                let valid_bb =
                                    self.context.append_basic_block(fn_value, "gen_valid");
                                let stale_bb =
                                    self.context.append_basic_block(fn_value, "gen_stale");

                                self.builder
                                    .build_conditional_branch(is_stale, stale_bb, valid_bb)
                                    .map_err(|e| {
                                        vec![Diagnostic::error(
                                ...
                                let actual_gen = if let Some(get_gen_fn) =
                                    self.module.get_function("blood_get_generation")
                                {
                                    let gen_result = self
                                        .builder
                                        .build_call(get_gen_fn, &[address.into()], "actual_gen")
                                        .map_err(|e| {
                                            vec![Diagnostic::error(
                                                format!("LLVM call error: {}", e),
                                                body.span,
                ...
                PlaceElem::Field(idx) => {
                        ...
                        if let Some(variants) = self.enum_defs.get(&enum_def_id) {
                            if let Some(variant_fields) = variants.get(variant_idx as usize) {
                                if let Some(variant_field_ty) = variant_fields.get(*idx as usize) {
                                    ...
                                    let actual_field_ty =
                                        self.substitute_type_params(variant_field_ty, &args);
                                    ...
                                    let variant_field_types: Vec<inkwell::types::BasicTypeEnum> =
                                        variant_fields
                                            .iter()
                                            .map(|f| {
                                                let substituted =
                                                    self.substitute_type_params(f, &args);
                    ...
                    if std::env::var("BLOOD_DEBUG_FIELD").is_ok() {
                        eprintln!(
                            "[Field] ===== FIELD ACCESS idx={}, actual_idx={} =====",
                            idx, actual_idx
                        );
                        eprintln!("[Field] current_ty: {:?}", current_ty);
                        eprintln!(
                            "[Field] current_ptr type: {:?}",
                            current_ptr.get_type().print_to_string()
                        );
                    }
                    ...
                    if std::env::var("BLOOD_DEBUG_FIELD").is_ok() {
                        ...
                        if let TypeKind::Adt { def_id, .. } = effective_ty.kind() {
                            if let Some(fields) = self.struct_defs.get(def_id) {
                                eprintln!(
                                    "[Field] struct_defs for def_id {:?} has {} fields",
                                    def_id,
                                    fields.len()
                                );
                                ...
                                eprintln!(
                                    "[Field] WARNING: def_id {:?} NOT found in struct_defs!",
                                    def_id
                                );
                            }
                        }
                    ...
                    if is_ref_to_struct {
                        ...
                        let loaded_val = self
                            .builder
                            .build_load(ptr_ty, current_ptr, "struct_ptr")
                            .map_err(|e| {
                                vec![Diagnostic::error(
                                    format!("LLVM load error: {}", e),
                                    body.span,
                        ...
                        let struct_ptr = match loaded_val {
                            ...
                            BasicValueEnum::IntValue(int_val) => {
                                ...
                                self.builder
                                    .build_int_to_ptr(int_val, opaque_ptr_ty, "struct_ptr_cast")
                                    .map_err(|e| {
                                        vec![Diagnostic::error(
                                            format!("LLVM int_to_ptr error: {}", e),
                                            body.span,
                        ...
                        if std::env::var("BLOOD_DEBUG_FIELD").is_ok() {
                            eprintln!(
                                "[Field] GEP (ref path): struct_ptr type = {:?}, actual_idx = {}",
                                struct_ptr.get_type().print_to_string(),
                                actual_idx
                            );
                        }
                        ...
                        let gep_result = self
                            ...
                            .map_err(|e| {
                                vec![Diagnostic::error(
                                    format!(
                                        "LLVM GEP error: {} (place={:?}, ty={:?})",
                                        e, place, effective_ty
                                    ),
                                    body.span,
                                )]
                            })?;
                        if std::env::var("BLOOD_DEBUG_FIELD").is_ok() {
                            eprintln!(
                                "[Field] GEP (ref path) result: {:?}",
                                gep_result.get_type().print_to_string()
                            );
                        }
                        gep_result
                    } else {
                ...
                PlaceElem::Index(idx_local) => {
                    ...
                    let idx_val = self
                        .builder
                        .build_load(idx_load_ty, *idx_ptr, "idx")
                        .map_err(|e| {
                            vec![Diagnostic::error(
                                format!("LLVM load error: {}", e),
                                body.span,
                    ...
                    if std::env::var("BLOOD_DEBUG_PLACE").is_ok() {
                        ...
                        eprintln!(
                            "[compile_mir_place] current_ty for IndexKind: {:?}",
                            current_ty
                        );
                    ...
                    if debug_vec {
                        eprintln!(
                            "[Index] Before type update: {:?}, index_kind: {:?}",
                            current_ty.kind(),
                            index_kind
                        );
                    }
                    ...
                    current_ty = match current_ty.kind() {
                        ...
                        TypeKind::Ref { inner, .. } | TypeKind::Ptr { inner, .. } => {
                            match inner.kind() {
                                    ...
                                    if Some(*def_id) == self.vec_def_id =>
                                {
                                    ...
                                    if debug_vec {
                                        eprintln!(
                                            "[Index] RefToVec: Vec def_id={:?}, elem type={:?}",
                                            def_id,
                                            elem.kind()
                                        );
                                    }
                        ...
                        TypeKind::Adt { def_id, args } if Some(*def_id) == self.vec_def_id => {
                            ...
                            if debug_vec {
                                eprintln!(
                                    "[Index] VecIndex: Vec def_id={:?}, elem type={:?}",
                                    def_id,
                                    elem.kind()
                                );
                            }
                    ...
                    unsafe {
                        match index_kind {
                            ...
                            IndexKind::DirectSlice => {
                                ...
                                let slice_llvm_ty =
                                    self.lower_type(&pre_index_ty).into_struct_type();
                                ...
                                let data_ptr = self
                                    .builder
                                    .build_load(ptr_ty, data_ptr_ptr, "slice_data_ptr")
                                    .map_err(|e| {
                                        vec![Diagnostic::error(
                                            format!("LLVM load error: {}", e),
                                            body.span,
                            ...
                            IndexKind::RefToArray => {
                                ...
                                let array_ptr = self
                                    .builder
                                    .build_load(ptr_ty, current_ptr, "array_ptr")
                                    .map_err(|e| {
                                        vec![Diagnostic::error(
                                            format!("LLVM load error: {}", e),
                                            body.span,
                            ...
                            IndexKind::PtrToElements => {
                                ...
                                let data_ptr = self
                                    .builder
                                    .build_load(ptr_ty, current_ptr, "data_ptr")
                                    .map_err(|e| {
                                        vec![Diagnostic::error(
                                            format!("LLVM load error: {}", e),
                                            body.span,
                                ...
                                if std::env::var("BLOOD_DEBUG_VEC_SIZE").is_ok() {
                                    ...
                                    eprintln!(
                                        "[GEP PtrToElements] HIR: {:?}, LLVM: {}, size: {}",
                                ...
                                let idx_i64 = if idx_int.get_type().get_bit_width() < 64 {
                                    self.builder
                                        .build_int_z_extend(idx_int, i64_type, "idx_i64")
                                        .map_err(|e| {
                                            vec![Diagnostic::error(
                                    ...
                                    self.builder
                                        .build_int_truncate(idx_int, i64_type, "idx_i64")
                                        .map_err(|e| {
                                            vec![Diagnostic::error(
                                ...
                                let byte_offset = self
                                    .builder
                                    .build_int_mul(idx_i64, elem_size_val, "byte_offset")
                                    .map_err(|e| {
                                        vec![Diagnostic::error(
                                            format!("LLVM mul error: {}", e),
                                            body.span,
                                        )]
                                    })?;
                            ...
                            IndexKind::VecIndex => {
                                ...
                                let data_ptr_ptr = self
                                    ...
                                    .map_err(|e| {
                                        vec![Diagnostic::error(
                                            format!("LLVM GEP error for Vec data pointer: {}", e),
                                            body.span,
                                        )]
                                    })?;
                                let ptr_ty = self.context.ptr_type(AddressSpace::default());
                                let data_ptr = self
                                    .builder
                                    .build_load(ptr_ty, data_ptr_ptr, "vec_data_ptr")
                                    .map_err(|e| {
                                        vec![Diagnostic::error(
                                            format!("LLVM load error: {}", e),
                                            body.span,
                                ...
                                if std::env::var("BLOOD_DEBUG_GEP_ADDR").is_ok() {
                                    if let Some(debug_fn) =
                                        self.module.get_function("debug_vec_index")
                                    {
                                ...
                                if std::env::var("BLOOD_DEBUG_GEP_ADDR").is_ok() {
                                    if let Some(debug_fn) =
                                        self.module.get_function("debug_vec_ptrs")
                                    {
                                        let _ = self.builder.build_call(
                                            debug_fn,
                                    ...
                                    if let Some(read_fn) =
                                        self.module.get_function("debug_read_enum_at")
                                    {
                                        let _ = self.builder.build_call(
                                            read_fn,
                                            &[elem_ptr.into()],
                                ...
                                if std::env::var("BLOOD_DEBUG_GEP_ADDR").is_ok() {
                                    eprintln!(
                                        "[VecIndex cast] elem_llvm_ty: {:?}",
                                        elem_llvm_ty.print_to_string()
                                    );
                                }
                            ...
                            IndexKind::RefToVec => {
                                ...
                                let vec_ptr = self
                                    .builder
                                    .build_load(ptr_ty, current_ptr, "vec_ref")
                                    .map_err(|e| {
                                        vec![Diagnostic::error(
                                            format!("LLVM load error: {}", e),
                                            body.span,
                                ...
                                let data_ptr_ptr = self
                                    .builder
                                    .build_struct_gep(inner_vec_ty, vec_ptr, 0, "vec_data_ptr_ptr")
                                    .map_err(|e| {
                                        vec![Diagnostic::error(
                                            format!("LLVM GEP error for Vec data pointer: {}", e),
                                            body.span,
                ...
                PlaceElem::ConstantIndex {
                    ...
                    let idx = if *from_end {
                        ...
                        let array_len = match effective_array_ty.kind() {
                            TypeKind::Array { size, .. } => size.as_u64().ok_or_else(|| {
                                vec![Diagnostic::error(
                            ...
                            _ => {
                                return Err(vec![Diagnostic::error(
                                    format!(
                                        "ConstantIndex from_end requires array type, got {:?}",
                                        current_ty
                                    ),
                                    body.span,
                                )]);
                    ...
                    unsafe {
                        if is_direct_array {
                            ...
                            let elem_llvm_ty = self.lower_type(&current_ty);
                            self.builder
                                .build_in_bounds_gep(elem_llvm_ty, current_ptr, &[idx], "const_idx")
                                .map_err(|e| {
                                    vec![Diagnostic::error(
                                        format!("LLVM GEP error: {}", e),
                                        body.span,
                ...
                PlaceElem::Subslice {
                    ...
                    unsafe {
                        if is_direct_array {
                            ...
                            self.builder
                                .build_in_bounds_gep(elem_llvm_ty, current_ptr, &[idx], "subslice")
                                .map_err(|e| {
                                    vec![Diagnostic::error(
                                        format!("LLVM GEP error: {}", e),
                                        body.span,
                    ...
                    match current_ty.kind() {
                        ...
                        TypeKind::Ref { inner, .. } | TypeKind::Ptr { inner, .. } => {
                            ...
                            if let TypeKind::Adt { def_id, .. } = inner.kind() {
                                ...
                                let enum_ptr = self
                                    .builder
                                    .build_load(ptr_ty, current_ptr, "enum_ref")
                                    .map_err(|e| {
                                        vec![Diagnostic::error(
                                            format!("LLVM load error: {}", e),
                                            body.span,
    ...
    fn compile_mir_place_load(
        ...
        let base_ty = match &place.base {
            ...
            crate::mir::types::PlaceBase::Static(def_id) => self
                .get_static_type(*def_id)
        ...
        let load_inst = self
            .builder
            .build_load(load_llvm_ty, ptr, "load")
            .map_err(|e| {
                vec![Diagnostic::error(
                    format!("LLVM load error: {}", e),
                    body.span,
        ...
        if std::env::var("BLOOD_DEBUG_LOAD").is_ok() {
            eprintln!(
                "[compile_mir_place_load] place_ty: {:?}, loaded_type: {:?}",
                place_ty,
                load_inst.get_type().print_to_string()
            );
        }
                ...
                PlaceElem::Field(idx) => {
                    ...
                    match current_ty.kind() {
                        ...
                        TypeKind::Adt { def_id, args } => {
                            ...
                            if variant_ctx.is_some()
                                && (Some(*def_id) == self.option_def_id
                                    || Some(*def_id) == self.result_def_id)
                            {
