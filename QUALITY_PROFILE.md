...
3. **Example and Entry Point Integrity** — Every example in `examples/` must compile and run. The README must accurately describe the project's current state. CLI `--help` must be complete and correct. A user following GETTING_STARTED.md must succeed without encountering broken paths. Test: CI runs `blood check` on every example; README claims are verified against reality.
