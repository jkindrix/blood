...
4. **Bootstrap Stability** — The self-hosting chain (blood-rust → first_gen → second_gen → third_gen) must remain byte-identical and all golden tests must pass. This is the project's core engineering achievement and regression here is catastrophic. Test: the existing build_selfhost.sh pipeline (already strong).
...
| Bootstrap Stability | 3-stage bootstrap, byte-identical second_gen/third_gen, 355/356 golden passing, CCV methodology | Excellent — this is the project's strongest dimension | Maintained at current level; zero golden regressions | 1 known failure (t06_err_import_error); otherwise strong |
