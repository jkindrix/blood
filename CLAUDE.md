...
**Runtime archive is compiler-generation-specific.** `build/libblood_runtime.a` is copied from `runtime/blood-runtime/build/debug/libblood_runtime_blood.a` (the `RUNTIME_A` variable). Building the Blood runtime with a different compiler generation produces a different archive. Using the wrong archive causes stale reference crashes during self-compilation (symptom: `panic: stale reference detected` in HIR lowering). **Before starting a build session**, verify: `md5sum bootstrap/libblood_runtime_blood.a` should match `build/libblood_runtime.a`. If they differ and runtime source hasn't changed, copy the bootstrap version: `cp bootstrap/libblood_runtime_blood.a runtime/blood-runtime/build/debug/ && cp bootstrap/libblood_runtime_blood.a src/selfhost/build/libblood_runtime.a`.
