...
**Runtime-evolution gotcha (bit S45, S50, S57).** When you intentionally edit runtime source (`runtime/blood-runtime/*.blood` or runtime FFI) and rebuild via `build blood_runtime`, the fresh archive lands in `runtime/blood-runtime/build/debug/libblood_runtime_blood.a` and `src/selfhost/build/libblood_runtime.a`. `bootstrap/libblood_runtime_blood.a` still holds the OLD hash. On the next selfhost build, `check_runtime_consistency()` sees the three archives diverging and "auto-fixes" by copying the bootstrap (OLD) copy over the new ones — silently reverting your runtime change. **To pin a fresh runtime**: `cp src/selfhost/build/libblood_runtime.a bootstrap/libblood_runtime_blood.a` BEFORE running `build first_gen` (or any command that triggers the consistency check). The gate pipeline handles this automatically on success, but ad-hoc runtime work between gates must propagate to `bootstrap/` manually.
